...
/* ofxLatk local patch: upstream only includes <unistd.h> when ZIP_HAVE_SYMLINK
   is defined, but ftruncate() and unlink() are used unconditionally further
   down. Under C99 and later that is a hard error rather than an implicit
   declaration, so the include is made unconditional here. See libs/zip/README.md */
#include <unistd.h> // ftruncate(), unlink(), symlink()
...
/* ofxLatk local patch: keep miniz from defining zlib-compatible names
   (Byte, uLong, Z_OK, ...), which would collide with the real zlib that
   openFrameworks links. zip.c itself only uses the mz_* names.
   See libs/zip/README.md */
